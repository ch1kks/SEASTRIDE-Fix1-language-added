        ...
        <div className="p-4 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="bg-[#2b1d19] border-4 border-[#b45309] rounded-2xl p-3 text-center space-y-1">
            ...
            <div className="text-xs text-[#fde68a] font-mono">
              ...
              ({currentServer.playerCount}/{currentServer.maxPlayers} {t("server.shipsCount")})
            </div>
          </div>
              ...
              {servers.map((s) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-[#fde68a]/80 font-mono">
                          {t("server.uniqueCode")}: {s.code} • {s.playerCount}/{s.maxPlayers} {t("server.shipsCount")}
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      {isCurrent ? (
                        <span className="text-xs font-black text-white flex items-center gap-1 bg-[#93bb44] border-b-4 border-[#658627] shadow-sm px-2.5 py-1 rounded-lg border border-[#064e3b]">
                          <Check className="w-3.5 h-3.5 text-white" /> {t("common.equipped")}
                        </span>
                      ) : (
          ...
          >
            <div className="flex justify-between items-center text-xs font-serif font-black uppercase text-[#fde68a]">
              <span className="flex items-center gap-1">
                <Plus className="w-3.5 h-3.5 text-[#facc15]" /> {t("server.createPrivateTitle")}
              </span>
            ...
            <input
              type="text"
              placeholder={t("server.islandNamePlaceholder")}
              value={newIslandName}
            ...
            >
              {t("server.createButton")} (10 {t("common.gems")})
            </button>
          </form>
        </div>
      </div>
